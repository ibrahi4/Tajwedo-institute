...
import { tajweedRules } from "@/config/tajweed";
...
import TajweedLevelSelector from "./TajweedLevelSelector";
import TajweedLearnPhase from "./TajweedLearnPhase";
import TajweedQuizPhase from "./TajweedQuizPhase";
...
export default function TajweedGame() {
  ...
  const currentRule: TajweedRule = tajweedRules[currentLevelIndex];
  ...
  const handleNextLevel = useCallback(() => {
    if (currentLevelIndex < tajweedRules.length - 1) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 via-white to-indigo-50">
      {phase !== "select" && (
        <GameHeader
          ...
          subtitle={currentRule ? `${currentRule.name} - ${currentRule.nameEn}` : ""}
          currentLevel={currentLevelIndex + 1}
          totalLevels={tajweedRules.length}
          score={score}
          showBack={true}
          backHref="/games"
        />
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === "learn" && currentRule && (
          <motion.div key="learn" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }}>
            <TajweedLearnPhase rule={currentRule} onStartQuiz={handleStartQuiz} />
          </motion.div>
        )}

        {phase === "quiz" && currentRule && (
          <motion.div key="quiz" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }}>
            <TajweedQuizPhase
              rule={currentRule}
              allRules={tajweedRules}
